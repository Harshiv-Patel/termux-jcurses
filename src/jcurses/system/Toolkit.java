...
/**
*	TERMUX_PREFIX is the prefix for termux environment. 
*	As termux runs on Android, it runs in prefixed environment. 
*	So instead of "/usr/local",
* 		we use path of termux's data directory.
*
* 		@see	#getLibraryPath()
*		@auther Harshiv Patel
*/
...
/**
* 		Instead of standard native library loading, ensure that libpath is initialized
* 		properly. Then initialize the jcurses library.
*/
  static {
   String path="";
   try{
	...
	if(path != null && !path.isEmpty()){
		...
		System.out.println("native libpath is null");
		System.exit(1);
...
/**
* This method returns appropriate absolute path of native library.
* Checks if running in Termux.
* If so, return standard Termux libpath.
* Otherwise follow standard procedure provided from github's "kba/jcurses" repo.
*
* @return a String object containing native library path
*/ 
  private static String getLibraryPath() {
	  ...
	  System.out.println("prefix: "+prefix);
		...
		if(TERMUX_PREFIX.equals(prefix)){
			return TERMUX_PREFIX+"/lib/libjcurses.so";
		}
